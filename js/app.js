 ...
 */            (function(global, factory) {
    ...
    (() => {
        ...
        let sliderPosts, sliderCourses;
        let init = false;
        function swiperSmall() {
            if (document.querySelector(".swiper-small")) if (window.innerWidth <= 600) {
                if (!init) {
                    init = true;
                    if (document.querySelector(".posts__content")) sliderPosts = new swiper_core_Swiper(".posts__content", {
                        modules: [ Navigation ],
                        observer: true,
                        observeParents: true,
                        slidesPerView: 1,
                    ...
                    if (document.querySelector(".courses-block__slider")) sliderCourses = new swiper_core_Swiper(".courses-block__slider", {
                        modules: [ Navigation ],
                        observer: true,
                        observeParents: true,
                        slidesPerView: 1.1,
                ...
                if (sliderPosts) sliderPosts.destroy();
                if (sliderCourses) sliderCourses.destroy();
                init = false;
            }
        }
        swiperSmall();
        ...
        jquery((function() {
            const $mainCoursesButton = jquery(".main-courses__button");
            const $mainCoursesItem = jquery(".item-search");
            const $searchForm = jquery(".search-form");
            const $searchFormInput = jquery(".search-form__input");
            const $mainCoursesItems = jquery(".items-search");
            const $pagging = jquery(".pagging");
            let noResultsMessage = null;
            $mainCoursesButton.on("click", (function() {
                const filterValue = jquery(this).data("filter");
                $pagging.hide();
                $mainCoursesItem.hide().filter(filterValue).slideDown(200);
                $mainCoursesButton.removeClass("active");
                jquery(this).addClass("active");
                hideNoResultsMessage();
            }));
            $searchForm.on("submit", (function(e) {
                e.preventDefault();
                const searchValue = $searchFormInput.val().toLowerCase().trim();
                const $matchingItems = $mainCoursesItem.hide().filter((function() {
                    const title = jquery(this).find(".item-search__title a").text().toLowerCase();
                    return searchValue.split(" ").every((word => title.includes(word)));
                })).slideDown(400);
                hideNoResultsMessage();
                if ($matchingItems.length === 0 && searchValue !== "") showNoResultsMessage();
                $mainCoursesButton.removeClass("active");
            }));
            $searchFormInput.on("input", (function() {
                const searchValue = jquery(this).val().toLowerCase().trim();
                const $items = $mainCoursesItem.hide();
                if (searchValue === "") {
                    $items.slideDown(400);
                    hideNoResultsMessage();
                }
                $mainCoursesButton.removeClass("active");
                $mainCoursesButton.filter('[data-filter="*"]').addClass("active");
            }));
            $searchFormInput.on("blur", (function() {
                jquery(this).val("");
            }));
            function showNoResultsMessage() {
                if (!noResultsMessage) noResultsMessage = jquery('<div class="no-results">No results found...</div>').appendTo($mainCoursesItems);
            }
